class Form {
  ...
  setElementsPosition() {
    ...
    this.BackButton.position(width / 2 - 200, height / 1 - -630);
    this.nextButton.position(width / 2 - -10, height / 1 - -630);
    this.greeting.position(width / 2 - 300, height / 2 - -445);
    this.Text.position(width / 2 - 200, height / 2 - -460);
    ...
    this.infoIcon.position(width / 2 + 580, height / 2 - -490);
  }
  ...
  validation() {
    ...
    if (isNaN(this.tvn)) {
      ...
      this.endText = createElement("h2");
      this.endLoginB = createButton("Login");
      this.endLoginB.class("customButton");
      this.endLoginB.position(width / 2 - 30, height / 2 - -730);
      this.endText.class("greeting4")
      this.endText.position(width / 2 - 590, height / 2 - -500);
      var newMess = `click the button below to check your details by giving your username and password`;
      this.endText.html(newMess);

      this.endLoginB.mousePressed(() =>{
        Swal.fire({
          title:'how to check the deatils',
          icon:'info',
          text: 'enter the correct username and password to get the detail'
        })
        upile = new userpasswordil();
        upile.display();
        this.endLoginB.hide();
        this.endText.hide();
      })
